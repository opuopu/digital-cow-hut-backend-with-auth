...
const createAorder = (cowsid, buyerid) =>
    ...
    if (buyerdata && cowsdata && cowsdata.label === 'for sale') {
      if (buyerdata.budget >= cowsdata.price) {
        ...
        try {
          ...
          yield cow_model_1.default.updateOne(
            { _id: cowsid },
            { $set: { label: 'sold out' } },
            { session }
          )
          neworderdata = yield order_model_1.order.create(
            [{ cow: cowsid, buyer: buyerid }],
            {
              session,
            }
          )
...
const getorders = () =>
  __awaiter(void 0, void 0, void 0, function* () {
    const result = yield order_model_1.order.find().populate('cow buyer')
    return result
  })
const orderservices = {
  createAorder,
